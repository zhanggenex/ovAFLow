...
void _loop_detect(ALGraph *cfg, ArcNode *an, int pre_bb_num) {
  //llvm::errs() << cfg->list[pre_bb_num].bb_name << "\n";
  VNode *vn;
...
void loop_detect(ALGraph *cfg) {
  ...
  for (temp = cfg->list[0].firarc; temp != NULL; temp = temp->nextarc) {
    //llvm::errs() << cfg->list[temp->adjvex].bb_name << "\n";
    ...
    for (auto &BB : F) {
      ...
      if (cfg.list[bb_num].loop_cnt > 0) {
		...
		for (auto Inst = BB.begin(); Inst != BB.end(); Inst++) {
		  ...
		  if (PHINode *PH = dyn_cast<PHINode>(Inst)) {
		    I = dyn_cast<Value>(PH);
		  }

		  if (Inst->getOpcode() == Instruction::Add && Inst->getOperand(0) == I) {
		    Add = dyn_cast<Value>(Inst);
		  }

		  if (CmpInst *CI = dyn_cast<CmpInst>(Inst)) {
		    if (Inst->getOperand(0) == Add && HasMem) {
			  
			  IRBuilder<> build(CI);

		      Input = dyn_cast<Value>(CI->getOperand(1));
			  inst_loop++;

			  unsigned int cur_loc_he = AFL_R(MAP_SIZE_HE);
			  ConstantInt *CurLocHe = ConstantInt::get(Int32Ty, cur_loc_he);

			  Value* Id = build.CreateAnd(CurLocHe, CurLocHe);

        	  LoadInst *LoopPtr = build.CreateLoad(AFLLoopPtr);
        	  LoopPtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
        	  Value *LoopPtrIdx = build.CreateGEP(LoopPtr, Id);

        	  Value *LoopIncr = build.CreateAdd(Input, ConstantInt::get(Int32Ty, 0));
